...
import { ArrowRight, Check, ChevronDown, CircleHelp, Clipboard, Clock3, Copy, ExternalLink, FileCheck2, LayoutDashboard, Link2, LogOut, Menu, Plus, RefreshCw, ShieldCheck, Sparkles, WalletCards, X, Zap } from 'lucide-react'
...
function AppShell({ wallet, children }: { wallet: ReturnType<typeof useWallet>; children: React.ReactNode }) {
...
function Overview({ campaigns, wallet, onRefresh }: { campaigns: Campaign[]; wallet: ReturnType<typeof useWallet>; onRefresh: () => void }) {
  ...
  const pending = campaigns.flatMap(c => c.slots.filter(s => s.status === 1)).length
...
function CampaignList({ campaigns }: { campaigns: Campaign[] }) { return <><PageHeading eyebrow="Campaign directory" title="Find open work." action={<Button to="/app/campaigns/new" icon={<Plus size={16} />}>New campaign</Button>} /><div className="list-intro"><span><span className="status-dot" /> {campaigns.filter(c => !isPast(c.deadline)).length} campaigns accepting slots</span><span className="mono">FIXED PAYOUT / OPEN ACCESS</span></div>{campaigns.length ? <div className="directory-list">{campaigns.map(c => <Link to={`/app/campaigns/${c.id}`} className="directory-row" key={c.id}><div className="directory-number">{String(c.id + 1).padStart(2, '0')}</div><div className="directory-main"><div><h3>{c.title}</h3><p>{c.brief || 'Fixed-payout creator campaign on Monad testnet.'}</p></div><div className="directory-meta"><span><b>{formatUnits(c.payout)}</b> USDC max / KOL</span><span><b>{Number(c.joined)} / {Number(c.maxSlots)}</b> slots</span><span className={isPast(c.deadline) ? 'muted-text' : 'green-text'}>{isPast(c.deadline) ? 'closed' : `ends ${fmtDate(c.deadline)}`}</span></div></div><ArrowRight size={17} /></Link>)}</div> : <EmptyState title="No live campaigns" body={isConfigured ? 'Create a campaign or check again after a new one is funded.' : 'Once the contract addresses are configured, funded campaigns will appear here.'} action={<Button to="/app/campaigns/new">Create campaign</Button>} />}</> }

function CreateCampaign({ wallet, onRefresh }: { wallet: ReturnType<typeof useWallet>; onRefresh: () => void }) {
  const navigate = useNavigate(); const [title, setTitle] = useState(''); const [brief, setBrief] = useState(''); const [payout, setPayout] = useState('100'); const [slots, setSlots] = useState('10'); const [days, setDays] = useState('14'); const [timeout, setTimeoutValue] = useState('48'); const [inviteOnly, setInviteOnly] = useState(false); const [inviteCode, setInviteCode] = useState(''); const [busy, setBusy] = useState(''); const [createdId, setCreatedId] = useState<string>(); const [message, setMessage] = useState('')
  const total = (Number(payout || 0) * Number(slots || 0)).toLocaleString()
  const create = async (event: React.FormEvent) => { event.preventDefault(); if (!wallet.account) return wallet.connect(); if (!USDC_ADDRESS) return setMessage('Add the Monad testnet USDC address to .env first.'); if (inviteOnly && inviteCode.trim().length < 6) return setMessage('Invite codes must be at least 6 characters.'); setBusy('Creating campaign'); setMessage(''); try { const deadline = BigInt(Math.floor(Date.now() / 1000) + Number(days) * 86400); const codeHash = inviteOnly ? keccak256(toBytes(inviteCode.trim())) : `0x${'0'.repeat(64)}` as `0x${string}`; const hash = await sendTransaction(wallet.account, 'createCampaign', [USDC_ADDRESS, title, brief, toUnits(payout), BigInt(slots), deadline, BigInt(Number(timeout) * 3600), inviteOnly, codeHash]); setMessage(`Campaign created. Transaction ${shortenAddress(hash)}`); setBusy(''); onRefresh(); setCreatedId('latest'); } catch (err) { setMessage(err instanceof Error ? err.message : 'Campaign creation failed.'); setBusy('') } }
  const fund = async () => { if (!wallet.account || !createdId) return; setBusy('Approving USDC'); try { const count = await publicClient.readContract({ address: ESCROW_ADDRESS, abi: escrowAbi, functionName: 'campaignCount' }) as bigint; const id = count - 1n; const amount = toUnits(total); await sendTransaction(wallet.account, 'approve', [ESCROW_ADDRESS, amount], true); setBusy('Locking USDC'); await sendTransaction(wallet.account, 'fundCampaign', [id]); setMessage('USDC approved and locked in escrow.'); setBusy(''); onRefresh(); navigate(`/app/campaigns/${id}`) } catch (err) { setMessage(err instanceof Error ? err.message : 'Funding failed.'); setBusy('') } }
  return <><PageHeading eyebrow="Agency / new campaign" title="Lock the budget first." action={<Button variant="quiet" to="/app/campaigns" icon={<X size={15} />}>Cancel</Button>} /><div className="form-layout"><form className="panel campaign-form" onSubmit={create}><div className="form-section"><span className="eyebrow">01 / Campaign brief</span><label>Campaign name<input required value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Launch week / creators" /></label><label>What should the KOL deliver?<textarea value={brief} onChange={e => setBrief(e.target.value)} placeholder="One public post on X with the campaign link and a clear mention." /></label></div><div className="form-section"><span className="eyebrow">02 / Fixed payout</span><div className="form-grid"><label>Maximum payment per KOL<div className="input-unit"><input required type="number" min="1" value={payout} onChange={e => setPayout(e.target.value)} /><span>USDC</span></div></label><label>Slots<input required type="number" min="1" value={slots} onChange={e => setSlots(e.target.value)} /></label></div><div className="budget-readout"><span>Total budget locked</span><b>{total} <small>USDC</small></b></div><p className="form-help">After a KOL joins, the agency can lower that slot's payout before proof is submitted.</p></div><div className="form-section"><span className="eyebrow">03 / Access</span><label className="check-row"><input type="checkbox" checked={inviteOnly} onChange={e => setInviteOnly(e.target.checked)} /><span>Invite-only campaign</span></label>{inviteOnly && <label>Campaign code<input required value={inviteCode} onChange={e => setInviteCode(e.target.value)} minLength={6} placeholder="Share this code with selected KOLs" /></label>}<p className="form-help">The code is a shareable bearer key. Anyone who has it can claim an available slot.</p></div><div className="form-section"><span className="eyebrow">04 / Timing</span><div className="form-grid"><label>Campaign duration<select value={days} onChange={e => setDays(e.target.value)}><option value="7">7 days</option><option value="14">14 days</option><option value="30">30 days</option></select></label><label>Review timeout<select value={timeout} onChange={e => setTimeoutValue(e.target.value)}><option value="24">24 hours</option><option value="48">48 hours</option><option value="72">72 hours</option></select></label></div><p className="form-help"><Clock3 size={14} /> If an agency does not review a submitted proof within this window, the KOL can claim the slot payout.</p></div>{message && <div className={message.includes('failed') || message.includes('Add') ? 'form-message error' : 'form-message'}>{message}</div>}<Button type="submit" disabled={Boolean(busy)}>{busy || 'Create campaign'}<ArrowRight size={16} /></Button></form><aside className="preview-panel"><span className="eyebrow">Read before signing</span><h3>{inviteOnly ? 'Fixed payout / invite access' : 'Fixed payout / open access'}</h3><p>{inviteOnly ? 'Only wallets with the campaign code can claim a slot.' : 'Anyone can claim one slot after the campaign is funded.'} The agency wallet is automatically blocked from becoming a KOL in its own campaign.</p><div className="preview-lines"><div><span>Network</span><b>Monad testnet</b></div><div><span>Settlement</span><b>USDC</b></div><div><span>Role model</span><b>Wallet / campaign</b></div></div>{createdId && <Button onClick={fund} disabled={Boolean(busy)}>{busy || 'Fund this campaign'}<Zap size={15} /></Button>}<div className="preview-note"><ShieldCheck size={15} /> Two signatures complete the setup: create, then fund.</div></aside></div></>
}

function CampaignDetail({ campaigns, wallet, onRefresh }: { campaigns: Campaign[]; wallet: ReturnType<typeof useWallet>; onRefresh: () => void }) {
  return <CampaignDetailV2 campaigns={campaigns} wallet={wallet} onRefresh={onRefresh} />
...
function App() { const wallet = useWallet(); const [refreshKey, setRefreshKey] = useState(0); const { campaigns, reload } = useCampaigns(refreshKey); const refresh = () => { setRefreshKey(x => x + 1); void reload() }; const appRoute = !wallet.ready ? <RouteLoading /> : wallet.account ? <AppShell wallet={wallet}><Routes><Route index element={<Overview campaigns={campaigns} wallet={wallet} onRefresh={refresh} />} /><Route path="campaigns" element={<CampaignList campaigns={campaigns} />} /><Route path="campaigns/new" element={<CreateCampaign wallet={wallet} onRefresh={refresh} />} /><Route path="campaigns/:id" element={<CampaignDetail campaigns={campaigns} wallet={wallet} onRefresh={refresh} />} /><Route path="campaigns/:id/proof" element={<PublicProof campaigns={campaigns} />} /><Route path="submissions" element={<ReviewQueue campaigns={campaigns} />} /><Route path="wallet" element={<WalletPage wallet={wallet} />} /></Routes></AppShell> : <Navigate to="/" replace />; return <Routes><Route path="/" element={<Landing wallet={wallet} />} /><Route path="/campaigns/:id/proof" element={<PublicProof campaigns={campaigns} />} /><Route path="/app/*" element={appRoute} /></Routes> }
