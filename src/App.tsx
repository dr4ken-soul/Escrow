...
function CreateCampaign({ wallet, onRefresh }: { wallet: ReturnType<typeof useWallet>; onRefresh: () => void }) {
  const navigate = useNavigate(); const [title, setTitle] = useState(''); const [brief, setBrief] = useState(''); const [payout, setPayout] = useState('100'); const [slots, setSlots] = useState('10'); const [days, setDays] = useState('14'); const [timeout, setTimeoutValue] = useState('48'); const [busy, setBusy] = useState(''); const [createdId, setCreatedId] = useState<number>(); const [createdCode, setCreatedCode] = useState(''); const [copied, setCopied] = useState(''); const [message, setMessage] = useState('')
  const total = (Number(payout || 0) * Number(slots || 0)).toLocaleString()
  const totalUnits = toUnits(payout || '0') * BigInt(slots || '0')
  const create = async (event: React.FormEvent) => { event.preventDefault(); if (!wallet.account) return wallet.connect(); if (!USDC_ADDRESS) return setMessage('Add the Monad testnet USDC address to .env first.'); setBusy('Creating campaign'); setMessage(''); try { const deadline = BigInt(Math.floor(Date.now() / 1000) + Number(days) * 86400); const code = genInviteCode(); const codeHash = keccak256(toBytes(code)); const hash = await sendTransaction(wallet.account, 'createCampaign', [USDC_ADDRESS, title, brief, toUnits(payout), BigInt(slots), deadline, BigInt(Number(timeout) * 3600), true, codeHash]); const count = await publicClient.readContract({ address: ESCROW_ADDRESS, abi: escrowAbi, functionName: 'campaignCount' }) as bigint; const id = Number(count - 1n); window.localStorage.setItem(`escrow:invite:${id}`, code); setCreatedCode(code); setMessage(`Campaign created. Transaction ${shortenAddress(hash)}`); setBusy(''); onRefresh(); setCreatedId(id); } catch (err) { setMessage(err instanceof Error ? err.message : 'Campaign creation failed.'); setBusy('') } }
  const fund = async () => { if (!wallet.account || createdId === undefined) return; setBusy('Approving USDC'); try { await sendTransaction(wallet.account, 'approve', [ESCROW_ADDRESS, totalUnits], true); setBusy('Locking USDC'); await sendTransaction(wallet.account, 'fundCampaign', [BigInt(createdId)]); setMessage('USDC approved and locked in escrow.'); setBusy(''); onRefresh(); navigate(`/app/campaigns/${createdId}`) } catch (err) { setMessage(err instanceof Error ? err.message : 'Funding failed.'); setBusy('') } }
