...
Copy the printed BatchEscrow address into `.env`, keep the Monad testnet USDC address above, and restart Vite. The agency wallet must hold testnet USDC and MON for the approval, funding, and gas transactions.

## Demo path

1. Agency wallet creates a campaign, for example 10 slots at 100 USDC.
...
4. Submit a public proof URL.
5. Switch back to the agency wallet and approve or reject the proof.
...
Invite-only campaigns, the 40/40/20 milestone payout preset, agency profile metadata, and social proof cards are documented in [FUTURE_UPDATES.md](./FUTURE_UPDATES.md). They are deliberately outside the V1 contract surface.
